...
if (isset($_POST['sign_up'])) {
    ...
    if ($rs_num_rows > 0) {
        ...
        $sql = "INSERT INTO users (name,university,course, security_question, security_answer,email,password,role) VALUES('$name','$university','$course','$security_question','$security_answer','$email','$password','$role')";
        $rs = mysqli_query($conn, $sql);
        if ($rs) {
            $_SESSION['message_success'] = "$role inserted successfully";
    ...
    $sql = "UPDATE users SET name = '$name', university = '$university', course = '$course', email = '$email' WHERE id = '" . $_SESSION['user_id'] . "'";
    $rs = mysqli_query($conn, $sql);
    if ($rs) {
    ...
    $rs = mysqli_query($conn, $sql);
    if ($rs) {
        header("location:post_and_view_question.php");
    } else {
        header("location:post_and_view_question.php");
    }
}
